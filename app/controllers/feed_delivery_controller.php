<?php
class FeedDeliveryController extends AppController {

	var $name = 'FeedDelivery';
	
	var $debug = 0;
	
	var $base_file_path = determineBaseFilePath();
	...
	function determineBaseFilePath(){
		...
		if($debug) echo "hostname: $hostname\n";
		if($debug) echo "base_file_path: $base_file_path\n";
	...
	function mkdir_recursive($pathname){
		global $debug;
		if($debug) echo "Making directory: $pathname\n";
	...
	function file_get_contents_to_disk($url, $file){
		...
		if(!is_dir($file_info['dirname']))
			mkdir_recursive($file_info['dirname']);
		...
		if(file_exists($file.'.tmp') && @simplexml_load_file($file) === FALSE){
			...
			if(file_exists($file.'.tmp'))
				unlink($file.'.tmp');
			setLastTimeUpdated($file);
		}
		return true;
	}

	function setLastTimeUpdated($feed){
		global $debug,$base_file_path;
		if(!is_dir($base_file_path))
			mkdir_recursive($base_file_path);
		$feed_update_time_file = $base_file_path.'last_update.json';
		if(file_exists($feed_update_time_file))
			$update_times = json_decode(file_get_contents($feed_update_time_file),1);
		$feed_stub = str_replace($base_file_path,'',$feed);
		if(substr($feed_stub,0,1) != '/')
			$feed_stub = '/'.$feed_stub;
		if($debug) echo "Setting last time updated for $feed_stub\n";
	...
	function updateFeed($sport=NULL,$league=NULL,$feed_type=NULL,$season=NULL){
		...
		$feed_path = getRemoteFeedStub($sport, $league, $feed_type, $season);
		$feed_path_update = getLastTimeUpdated($feed_path);
		$feeds_to_update = feedsToUpdate($feed_path,$feed_path_update);
		$feeds_fetched_count = fetchFeeds($feeds_to_update);
		$feeds_time_updated_happened = setLastTimeUpdated($feed_path);
		return $feeds_fetched_count;
	}

	function fetchAtomFeed($feed,$newerThan=NULL){
		global $base_url_atom;

		$url = $base_url_atom.'?feed='.$feed;
		if(!empty($newerThan)){
	...
	function getRemoteFeedStub($sport=NULL,$league=NULL,$feed_type=NULL,$season=NULL){
		...
		if(!empty($sport)){
			...
			if(!in_array($sport, $feedKeywords['sports'])){
				if($debug) die("Invalid sport: $sport");
				return false;
			}
		}
		...
		if(!empty($league)){
			...
			if(!array_key_exists($league, $feedKeywords['leagues'])){
				if($debug) die("Invalid league: $league");
				return false;
			}
		}
		...
		if(!empty($feed_type)){
			...
			if(!in_array($feed_type, $feedKeywords['feed_types'])){
				if($debug) die("Invalid feed type: $feed_type");
				return false;
			}
		}
		...
		if(!empty($season)){
			...
			if(!in_array($season, $feedKeywords['seasons'])){
				if($debug) die("Invalid season: $season");
				return false;
			}
		}
		return $feed_path;
	...
	function feedsToUpdate($feed,$newerThan=NULL){
		if(empty($feed))
			return false;
		global $base_url;
		$feedsToUpdateXML = simplexml_load_string(fetchAtomFeed($feed,$newerThan));
		$i=0;
		foreach($feedsToUpdateXML->entry as $feed){
			$url = (string)$feed->link->attributes()->href;
			$save_path = str_replace($base_url,'',$url);
	...
	function fetchFeeds($feedsToUpdate=NULL){
		...
		foreach($feedsToUpdate as $feed){
			if(file_get_contents_to_disk($feed['url'],$base_file_path.$feed['save_path']) !== FALSE)
				$i++;
		}
		return $i;
	}
	...
	function cleanOldFeeds($feed,$olderThan=604800){
		global $base_file_path,$debug;

		//Get the directory listing
		if ($handle = opendir($base_file_path.substr($feed,1))) {
			//Loop over the results
			while (false !== ($file = readdir($handle))) {
				//Skip . and ..
				if($file == '.' || $file == '..')
					continue;
				if($olderThan < (time()-filemtime($base_file_path.substr($feed,1).$file))){
					unlink($base_file_path.substr($feed,1).$file);
				}
			}
	...
	function startLiveFeedsProcess(){
		global $base_file_path;
		$pid_file = $base_file_path.'../../cron/pid.json';

		exec("nohup php -f live_feed_checker.php > /dev/null 2>&1 & echo $!",$op);
		$pid = (int)$op[0];
	...
	function pidRunning($checkPid=0){
		if(!is_numeric($checkPid) || $checkPid > 99999 || $checkPid == 0)
			return false;
		$ps_result = exec('ps '.$checkPid.' | grep live_feed_checker | wc | awk \'{print $1}\'');
		if($ps_result == 1)
			return true;
		return false;
	}
	...
	function index(){
		//Injury
		updateFeed('football','ncaaf','injuries');

		//Player
		updateFeed('football','ncaaf','players','2009-2010');

		//Player Stats
		updateFeed('football','ncaaf','player-stats','2009-2010');

		//Results
		updateFeed('football','ncaaf','results','2009-2010');

		//Schedule
		updateFeed('football','ncaaf','schedules');

		//Team
		updateFeed('football','ncaaf','teams','2009-2010');

		//Team Stats
		updateFeed('football','ncaaf','team-stats','2009-2010');

		//News
		updateFeed('football','ncaaf','news'); //This has been empty for a while 09-22-09 :(

		//Odds
		updateFeed('football','ncaaf','odds');

		//Consensus
		updateFeed('football','ncaaf','consensus');

		//Boxscores
		updateFeed('football','ncaaf','boxscores','2009-2010');

		//In-progress boxscores
		updateFeed('football','ncaaf','in-progress-boxscores');

		//Live Scores
		updateFeed('football','ncaaf','livescores');
